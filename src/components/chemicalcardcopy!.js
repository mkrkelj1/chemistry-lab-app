...
const ChemicalsCard = ( { chemicals, pictograms, pictogram_test_obj } ) => {


		const createImageElems = ( chemicalID ) => {
	const pictograms = pictogram_test_obj[chemicalID]
	return pictograms.map((pictogram) => 
		<React.Fragment> <Card.Img variant="buttom" src = { root_path + pictogram.filename } style = { image_style} /></React.Fragment>
	)
};
	...
	const chemicalCard = chemicals.map((chemical, i) =>
		<React.Fragment>
			...
			{createImageElems(chemical.chemicalID)}
			<hr />
		</React.Fragment>
	);
